...
def run(
    ...
    with sync_playwright() as playwright:
        bundle = get_browser_context_for_task(
            ...
            payload=payload,
            executor_name="smart_sherpa_sync",
            headless=headless,
            start_url=start_url or None,
            logger=print,
        )
        browser: Browser | None = bundle["browser"]
        context: BrowserContext | None = bundle["context"]
        ...
        if attach_to_existing and browser is not None:
            ...
            elif require_existing_page and not allow_launch_fallback:
                raise RuntimeError(
        ...
        try:
            ...
            return {
                ...
                "browser_profile_policy": bundle["profile"]["browser_profile_policy"],
                "bill_profile_ready": bundle["bill_profile_ready"],
                "bookmarks_ready": bundle["bookmarks_ready"],
                "debug_port_ready": bundle["debug_port_ready"],
                "profile_path": str(bundle["profile"]["bill_chrome_profile_dir"]),
                "profile_directory": str(bundle["profile"]["chrome_profile_directory"]),
                "remote_debugging_port": int(bundle["profile"]["remote_debugging_port"]),
                "browser_mode_selected": str(bundle["profile"]["browser_mode_selected"]),
                "executor_name": "smart_sherpa_sync",
