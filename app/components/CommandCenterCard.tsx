...
interface CommandCenterCardProps {
  ...
  setAutoSubmitVoiceCommands: Dispatch<SetStateAction<boolean>>;
  ...
  return (
    <section className="rounded-2xl border border-cyan-500/25 bg-slate-900/80 p-5 shadow-[0_24px_45px_-30px_rgba(8,145,178,0.7)]">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        <button
          type="button"
          onClick={() => setAutoSubmitVoiceCommands((v) => !v)}
          className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition ${
            autoSubmitVoiceCommands
              ? "border-emerald-400/40 bg-emerald-500/15 text-emerald-300"
              : "border-slate-700 bg-slate-900 text-slate-500 hover:text-slate-300"
          }`}
          title="Auto-submit final voice transcripts"
        >
          {autoSubmitVoiceCommands ? "Auto-Submit Voice On" : "Auto-Submit Voice Off"}
        </button>
