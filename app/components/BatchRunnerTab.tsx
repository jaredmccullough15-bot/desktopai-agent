...
type BatchRow = {
  ...
  status: string;
  payment_status: "good" | "bad" | "needs_review";
  task_id?: string | null;
...
export function BatchRunnerTab({ machines }: BatchRunnerTabProps) {
  ...
  const [workflowName, setWorkflowName] = useState("ci_checks");
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-slate-400">
          Upload CI Checks spreadsheets, map columns, run one child task per row on a selected worker, and export results.
        </p>
      </div>
              ...
              <tbody>
                {filteredRows.map((row) => (
                  <tr key={row.row_id} className="border-t border-slate-800">
                    ...
                    <td className="px-3 py-2">{row.mapped?.member_name || ""}</td>
                    <td className="px-3 py-2">{row.status}</td>
                    <td className="px-3 py-2">{row.matched_client_name || "-"}</td>
                    <td className="px-3 py-2">{row.mapped?.paid_through_date || "-"}</td>
