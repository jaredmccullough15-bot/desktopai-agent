...
$uiCmdPath = Join-Path $repo "run_worker_ui.cmd"
if (!(Test-Path $uiCmdPath)) {
    @"
@echo off
setlocal
set "SCRIPT_DIR=%~dp0"
...
endlocal
"@ | Set-Content -Path $uiCmdPath -Encoding ASCII
    Write-Host "Created missing run_worker_ui.cmd" -ForegroundColor Green
}

$uiPs1Path = Join-Path $repo "run_worker_ui.ps1"
if (!(Test-Path $uiPs1Path)) {
    @'
$ErrorActionPreference = 'Stop'
...
'@ | Set-Content -Path $uiPs1Path -Encoding UTF8
    Write-Host "Created missing run_worker_ui.ps1" -ForegroundColor Green
}
