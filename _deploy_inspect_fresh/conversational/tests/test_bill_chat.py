...
def _default_start_teaching_mode_stub(**kwargs: Any) -> dict[str, Any]:
    workflow_name = kwargs.get("workflow_name") or "PatientIntake"
    target_worker = kwargs.get("target_machine_uuid") or "uuid-worker-1"
    if not kwargs.get("workflow_name") and "called" not in str(kwargs.get("message") or "").lower():
        return {
    ...
    if not target_worker:
        return {
            ...
            "reply": "Which worker should I use?",
            "task_id": None,
            "draft_id": None,
            "session_id": None,
    ...
    return {
        ...
        "reply": f"Started teaching workflow '{workflow_name}'.",
...
def _make_service(
    ...
    start_teaching_mode_stub: Any | None = None,
    ...
    service = BillChatService(
        ...
        start_teaching_mode_fn=start_teaching_mode_stub or _default_start_teaching_mode_stub,
    )
...
def test_start_new_workflow_no_name_asks_for_name() -> None:
    ...
    assert ts.calls == [], "No direct task queueing should occur in BillChatService"
...
def test_start_new_workflow_with_name_queues_teach_session() -> None:
    ...
    assert response.teaching_mode is not None
...
def test_start_teach_session_queues_task() -> None:
    service, ts, _ = _make_service()
    response = service.handle_message(_req("teach bill this workflow called ClaimsIntake"))
...
def test_task_queued_with_target_machine_passes_field() -> None:
    """target_machine_uuid is forwarded to canonical startup helper."""
    service, ts, _ = _make_service()
    response = service.handle_message(_req("start a new workflow called X", target_machine_uuid="uuid-worker-1"))
    assert response.teaching_mode is not None
    assert response.teaching_mode.get("target_machine_uuid") == "uuid-worker-1"
    assert len(ts.calls) == 0
