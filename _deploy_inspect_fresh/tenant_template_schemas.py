...
class TenantWorkflowTemplate(BaseModel):
    ...
    # Input validation — workflow-specific required fields
    # required_inputs: arbitrary field names that must be present in the run payload.
    # identity_required: if True, the fields in identity_fields (or the global defaults)
    #   must all be present before the workflow is allowed to execute.
    # identity_fields: explicit list of required identity field names when identity_required=True.
    #   Falls back to the global _IDENTITY_FIELDS tuple if left empty.
