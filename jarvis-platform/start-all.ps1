...
$coreCommand = "Set-Location -Path `"$corePath`"; `$env:BILL_CORE_HOST=`"0.0.0.0`"; `$env:BILL_CORE_PORT=`"8000`"; `$env:BILL_WORKER_LATEST_VERSION=`"0.3.20`"; `$env:BILL_WORKER_PACKAGE_FILE=`"$workerPkgFile`"; `$env:BILL_WORKER_PACKAGE_PUBLIC_URL=`"https://api.bill-core.com/worker/update/package`"; `$env:BILL_WORKER_FORCE_UPDATE=`"true`"; & `"$corePythonExe`" -m uvicorn main:app --reload --host 0.0.0.0 --port 8000"
...
$coreCommand = "Set-Location -Path `"$corePath`"; `$env:BILL_CORE_HOST=`"0.0.0.0`"; `$env:BILL_CORE_PORT=`"8010`"; `$env:BILL_WORKER_LATEST_VERSION=`"0.3.20`"; `$env:BILL_WORKER_PACKAGE_FILE=`"$workerPkgFile`"; `$env:BILL_WORKER_PACKAGE_PUBLIC_URL=`"https://api.bill-core.com/worker/update/package`"; `$env:BILL_WORKER_FORCE_UPDATE=`"true`"; & `"$corePythonExe`" -m uvicorn main:app --reload --host 0.0.0.0 --port 8010"
